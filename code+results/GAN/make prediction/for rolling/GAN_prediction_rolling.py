...
y_train = df1[['Rolling_Fric']]
x_train = df1[['Size','Angle_left','Angle_right','Restitution','Density','Speed','Static_Fric']]
#scalar = MinMaxScaler()
#scalar.fit(x_train)
...
y_test = df2[['Rolling_Fric']].values
x_test = df2[['Size','Angle_left','Angle_right','Restitution','Density','Speed','Static_Fric']].values
...
def build_generator():
    model = Sequential()
    model.add(Dense(units=1024, input_dim=7))
...
def build_discriminator():
    model = Sequential()
    model.add(Dense(units=256, input_dim=1))
    model.add(LeakyReLU(alpha=0.01))
...
def get_accuracy(y_train,fake_data):
    n = y_train.shape[0]
    acc_tem = K.abs(y_train - fake_data) / y_train
    Acc_roll_fric = (1 - K.sqrt(K.sum(acc_tem * acc_tem) / n)) * 100
...
def plt_results():
    ...
    pyplot.scatter(generated_data, y_test, color='blue')
    pyplot.scatter(y_test, y_test, color='r')
    plt.xlabel('generated_data')
    plt.ylabel('y_test')
    plt.title('Compare predicted value with true value for Roll Friction after training')
...
def train_GAN(epochs,batch_size):
    ...
    for i in range(1, epochs+1): 
        for _ in tqdm(range(batch_size)):
            ...
            noise = train_random[['Size','Angle_left','Angle_right','Restitution','Density','Speed','Static_Fric']].values
            fake_data = generator.predict(noise)
            fake_data = fake_data
            
            # Select real data from dataset
            y_train = train_random[['Rolling_Fric']].values
            real_data = y_train
            ...
            noise2 = train_random2[['Size','Angle_left','Angle_right','Restitution','Density','Speed','Static_Fric']].values
            # Create the opposite label for fake data
   ...
   # plot the accuracy of discriminator
    ...
    plt.ylabel('accuracy')
    plt.legend(['g_acc'])
    plt.savefig('/home/hashemi/Fei Shao/GAN results/acc g.png')
    plt.show()
